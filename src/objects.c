...
#define O_POWER(...)	C17(({ __VA_ARGS__ }))
...
#define SUIT(names,kn,mgc,size,dexc,prob,delay,wt,cost,ac,dr,can,metal,c,...) \
	ARMOR(names, kn, mgc, size, dexc, {0}, prob, delay, wt, cost, ac, dr, can, TORSO_DR, ARM_SUIT, metal, c, __VA_ARGS__)
#define SHIRT(names,kn,mgc,size,prob,delay,wt,cost,ac,dr,can,metal,c,...) \
	ARMOR(names, kn, mgc, size, 0, {0}, prob, delay, wt, cost, ac, dr, can, UPPER_TORSO_DR, ARM_SHIRT, metal, c, __VA_ARGS__)
#define HELM(names,kn,mgc,prob,delay,wt,cost,ac,dr,can,metal,c,...) \
	ARMOR(names, kn, mgc, MZ_SMALL, 0, {0}, prob, delay, wt, cost, ac, dr, can, HEAD_DR, ARM_HELM, metal, c, __VA_ARGS__)
#define CLOAK(names,kn,mgc,prob,delay,wt,cost,ac,dr,can,metal,c,...) \
	ARMOR(names, kn, mgc, MZ_MEDIUM, 0, {0}, prob, delay, wt, cost, ac, dr, can, CLOAK_DR, ARM_CLOAK, metal, c, __VA_ARGS__)
#define SHIELD(names,kn,mgc,size,prob,delay,wt,cost,ac,dr,can,metal,c,...) \
	ARMOR(names, kn, mgc, size, 0, {0}, prob, delay, wt, cost, ac, dr, can, 0, ARM_SHIELD, metal, c, __VA_ARGS__)
#define GLOVES(names,kn,mgc,prob,delay,wt,cost,ac,dr,can,metal,c,...) \
...
HELM(("sedge hat", "wide conical hat"), /*Needs encyc entry*//*Needs tile*/
		0, 0,  3, 1,  3,   8,10, 0, 1, VEGGY, CLR_YELLOW),
HELM(("wide hat", "wide-brimmed hat"), /*Needs encyc entry*//*Needs tile*/
		0, 0,  0, 1,  3,   8,10, 0, 1, CLOTH, CLR_WHITE),
HELM(("leather helm", "leather hat"),
		0, 0,  5, 1,  5,   8,10, 1, 0, LEATHER, HI_LEATHER),
HELM(("orcish helm", "skull cap"),
		0, 0,  5, 1, 30,  10, 9, 0, 0, IRON, CLR_BLACK),
HELM(("dwarvish helm", "hard hat"),
		0, 0,  5, 1, 40,  20, 9, 1, 0, IRON, HI_METAL),
HELM(("gnomish pointy hat", "conical hat"),
		0, 0,  0, 1,  3,   2,10, 0, 0, CLOTH, CLR_RED),
HELM(("sunlight maggot", "parasitic insectoid"),
...
HELM(("witch hat", "wide-brimmed conical hat"),
		0, 1,  0, 1,  4,  80,10, 0, 2, CLOTH, CLR_BLACK),
HELM(("dunce cap", "conical hat"),
		0, 1,  3, 1,  4,   1, 10, 0, 0, CLOTH, CLR_BLUE),
HELM(("war hat", "wide helm"), /*Needs encyc entry*//*Needs tile*/
		0, 0,  2, 0, 60,  30, 8, 4, 2, IRON, HI_METAL),
HELM(("flack helmet", "green bowl", "bowl"), /*Needs encyc entry*/
		0, 0,  0, 0, 10,  50, 9, 3, 1, PLASTIC, CLR_GREEN),
...
HELM(("helmet", "plumed helmet"), /* circlet */
		0, 0,  8, 1, 30,  10, 9, 3, 0, IRON, HI_METAL),
HELM(("helm of brilliance", "etched helmet"), /* crown of cognizance */
		0, 1,  6, 1, 30,  50, 9, 1, 0, IRON, HI_METAL),
...
HELM(("helm of telepathy", "visored helmet"), /*tiara of telepathy*/ /*Note: 'visored' appearance gives +1 AC and DR*/
		0, 1,  2, 1, 30,  50, 9, 1, 0, IRON, HI_METAL, O_POWER(TELEPAT)),
HELM(("helm of drain resistance", "band"), /*diadem of drain resistance*//*Needs tile*//*Note: 'band' appearance halves AC and DR after adjustments for errosion and material (rounds down) */
		0, 1,  2, 1, 30,  50, 9, 1, 0, GOLD, HI_GOLD, O_POWER(DRAIN_RES)),
 ...
 */
#define DRGN_MAIL(names,mgc,cost,ac,dr,color,...) \
	SUIT(DEF_BLINDNAME(names, "dragon scale mail"),1,mgc, MZ_LARGE, ARMSZ_MEDIUM,0,5,150,cost,ac,dr,0,DRAGON_HIDE,color,__VA_ARGS__)
...
#define DRGN_SCALES(names,mgc,cost,ac,dr,color,...) \
	SUIT(DEF_BLINDNAME(names, "dragon scales"),1,mgc, MZ_LARGE, ARMSZ_MEDIUM,0,5,150,cost,ac,dr,0,DRAGON_HIDE,color,__VA_ARGS__)
...
SUIT(("elven toga"), /*Needs encyc entry*//*Needs tile*/
	1, 0,  MZ_LARGE,  ARMSZ_LIGHT,  0,  5,   5,  100, 10, 1, 2, CLOTH, CLR_GREEN),
SUIT(("noble's dress", "armored dress"), /*Needs encyc entry*/
	0, 0,  MZ_LARGE, ARMSZ_MEDIUM,  0,  5,  40, 2000,  6, 4, 3, SHADOWSTEEL, CLR_BLACK),
...
SUIT(("droven chain mail", "crested black mail", "crested mail"), /*Needs encyc entry*/
	0, 0,  MZ_LARGE, ARMSZ_MEDIUM,  0,  5,  50, 1000,  6, 5, 2, SHADOWSTEEL, CLR_BLACK),
SUIT(("orcish chain mail", "crude chain mail"),
	0, 0,  MZ_LARGE,  ARMSZ_HEAVY, 20,  5, 150,   75,  7, 3, 1, IRON, CLR_BLACK),
SUIT(("scale mail"),
	1, 0,  MZ_LARGE, ARMSZ_MEDIUM, 72,  5, 125,   45,  8, 3, 0, IRON, HI_METAL),
SUIT(("studded leather armor"),
	1, 0,  MZ_LARGE,  ARMSZ_LIGHT, 72,  3,  50,   15,  9, 2, 1, LEATHER, HI_LEATHER),
SUIT(("ring mail"),
	1, 0,  MZ_LARGE, ARMSZ_MEDIUM, 72,  5, 125,  100,  9, 3, 1, IRON, HI_METAL),
SUIT(("orcish ring mail", "crude ring mail"),
	0, 0,  MZ_LARGE, ARMSZ_MEDIUM, 20,  5, 125,   80,  9, 2, 1, IRON, CLR_BLACK),
SUIT(("leather armor"),
	1, 0,  MZ_LARGE,  ARMSZ_LIGHT, 82,  3,  40,    5, 10, 2, 0, LEATHER, HI_LEATHER),
//ARMOR(names,
   //kn,mgc,blk,prob,delay,wt,cost,ac,dr,can,sub,metal,c)
SUIT(("living armor", "giant sea anemone"),
	0, 1,  MZ_LARGE, ARMSZ_MEDIUM,  0,  6,  80,  500, 10, 2, 0, FLESH, CLR_ORANGE),
SUIT(("jacket"),
	1, 0, MZ_MEDIUM,  ARMSZ_LIGHT, 12,  0,	20,   10, 10, 1, 0, LEATHER, HI_LEATHER, O_MATSPEC(IDED|UNIDED)),
SUIT(("straitjacket", "long-sleeved jacket"), /*Needs encyc entry*//*Needs tile*/
	0, 0, MZ_MEDIUM,  ARMSZ_HEAVY,  0,  0,  15,   10, 10, 1, 2, CLOTH, CLR_WHITE, O_DRSLOT(UPPER_TORSO_DR|ARM_DR)),
...
SUIT(("Hawaiian shorts", "flowery short pants", "short pants"), /*Needs encyc entry*//*Needs tile*/
	0, 0, MZ_SMALL,  ARMSZ_LIGHT,   0,  0,  15,   10, 10, 0, 0, CLOTH, CLR_ORANGE, O_DRSLOT(LOWER_TORSO_DR)),
SHIRT(("Hawaiian shirt", "flowery shirt", "shirt"),
	0, 0, MZ_MEDIUM, 10, 0,	 5,   3, 10, 0, 0, CLOTH, CLR_MAGENTA),
SHIRT(("T-shirt"), /*Needs encyc entry*/
	1, 0, MZ_MEDIUM,  5, 0,	 5,   2, 10, 0, 0, CLOTH, CLR_WHITE),
...
SHIRT(("victorian underwear", "frilly dress"),
	0, 0, MZ_MEDIUM,  0, 5,   5,  10, 10, 2, 3, CLOTH, CLR_WHITE,  O_DRSLOT(TORSO_DR)),
SHIRT(("bodyglove", "tight clothes"), /*Needs encyc entry*//*Needs tile*/
	0, 0, MZ_HUGE,    0, 5,   5,1000, 10, 0, 3, PLASTIC, CLR_BLACK, O_POWER(SICK_RES), O_DRSLOT(ALL_DR)),
/* cloaks */
/*  'cope' is not a spelling mistake... leave it be */
CLOAK(("mummy wrapping"),
		1, 0,	0, 0,  3,  2, 10, 0, 1, CLOTH, CLR_GRAY, O_DRSLOT(ALL_DR)),
CLOAK(("prayer-warded wrapping"),
		1, 0,	0, 0,  3, 100, 10, 0, 1, CLOTH, CLR_GRAY, O_DRSLOT(ALL_DR)),
...
CLOAK(("white faceless robe", (char *)0, "faceless robe"),
		1, 1,	0, 2, 20, 50, 10, 1, 3, CLOTH, CLR_WHITE, O_DRSLOT(HEAD_DR|CLOAK_DR)),
CLOAK(("black faceless robe", (char *)0, "faceless robe"),
		1, 1,	0, 2, 20, 50, 10, 2, 3, CLOTH, CLR_BLACK, O_DRSLOT(HEAD_DR|CLOAK_DR), O_POWER(COLD_RES)),
CLOAK(("smoky violet faceless robe", (char *)0, "faceless robe"),
		1, 1,	0, 2, 20,500, 10, 3, 3, CLOTH, CLR_MAGENTA, O_DRSLOT(HEAD_DR|CLOAK_DR), O_POWER(COLD_RES)),
CLOAK(("alchemy smock", "apron"),
		0, 1,	9, 0, 10, 50, 10, 1, 3, CLOTH, CLR_WHITE, O_POWER(POISON_RES, ACID_RES)),
CLOAK(("Leo Nemaeus hide", "lion skin"),
		0, 1,	0, 10, 60, 1200, 10, 5, 0, DRAGON_HIDE, HI_GOLD, O_POWER(HALF_PHDAM)),
CLOAK(("cloak"),
		1, 0,	8, 0, 15, 40, 10, 2, 1, LEATHER, CLR_BROWN, O_MATSPEC(IDED|UNIDED)),
CLOAK(("studded leather cloak"),
		1, 0,	0, 0, 45, 80,  9, 2, 3, LEATHER, HI_COPPER),
/* With shuffled appearances... */
CLOAK(("cloak of protection", "tattered cape"),
		0, 1,	9, 0, 10, 50,  9, 1, 3, CLOTH, HI_CLOTH, O_POWER(PROTECTION)),
CLOAK(("cloak of invisibility", "opera cloak"),
		0, 1,  10, 0, 10, 60,  9, 0, 2, CLOTH, CLR_BLACK, O_POWER(INVIS)),
CLOAK(("cloak of magic resistance", "ornamental cope"),
		0, 1,	2, 0, 10, 60, 10, 0, 3, CLOTH, CLR_WHITE, O_POWER(ANTIMAGIC)),
CLOAK(("cloak of displacement", "piece of cloth"),
		0, 1,  10, 0, 10, 50,  9, 0, 2, CLOTH, CLR_BRIGHT_MAGENTA, O_POWER(DISPLACED)),
...
SHIELD(("Uruk-hai shield", "white-handed shield", "shield"),
		0, 0, MZ_MEDIUM,  2, 0, 50,	7,  8, 0, 1, IRON, HI_METAL),
SHIELD(("orcish shield", "red-eyed shield", "shield"),
		0, 0, MZ_MEDIUM,  2, 0, 50,	7,  9, 0, 0, IRON, CLR_RED),
SHIELD(("kite shield"),
		1, 0,  MZ_LARGE,  5, 0,100, 10,  8, 0, 1, IRON, HI_METAL),
SHIELD(("tower shield"),
		1, 0,  MZ_HUGE,   1, 0,200, 20,  6, 0, 1, IRON, HI_METAL),
SHIELD(("roundshield", "round shield"),
		0, 0,  MZ_LARGE,  1, 0,120,  7,  8, 0, 1, COPPER, HI_COPPER, O_MATSPEC(IDED)),
SHIELD(("dwarvish roundshield", "round shield"),
		0, 0,  MZ_LARGE,  4, 0, 80, 10,  7, 0, 1, IRON, HI_METAL),
SHIELD(("crystal shield", "shield"), /*Needs encyc entry*//*Needs tile*/
		0, 0,  MZ_LARGE,  0, 0, 80,150,  9, 0, 0, GLASS, HI_GLASS, O_MATSPEC(UNIDED)),
SHIELD(("shield of reflection", "polished shield", "smooth shield"),
		0, 1,  MZ_LARGE,  3, 0, 60, 50,  8, 0, 0, SILVER, HI_SILVER, O_POWER(REFLECTING)),
...
#define DRGN_SHIELD(names,mgc,cost,ac,dr,color,...)						\
	SHIELD(DEF_BLINDNAME(names, "dragon scale shield"),1,mgc,MZ_LARGE,0,0,75,cost,ac,dr,0,DRAGON_HIDE,color,__VA_ARGS__)
/* 3.4.1: dragon scale mail reclassified as "magic" since magic is
   needed to create them */
...
SHIELD(("stone dragon shield"),
		1, 1,  MZ_LARGE, 0, 0, 75, 1200,  7, 0, 0, MINERAL, CLR_GRAY),
 ...
 */
//define GLOVES(names,kn,mgc,prob,delay,wt,cost,ac,dr,can,metal,c) \
	ARMOR(names,kn,mgc,0,prob,delay,wt,cost,ac,dr,can,ARM_GLOVES,metal,c)
...
GLOVES(("gloves", "old gloves"),
		0, 0,  8, 1, 10,  8, 10, 1, 0, LEATHER, HI_LEATHER, O_MATSPEC(IDED)),
GLOVES(("gauntlets of fumbling", "padded gloves"), /*"padded" should give +1 DR*/
		0, 1,  7, 1, 10, 50, 10, 1, 0, LEATHER, HI_LEATHER, O_POWER(FUMBLING)),
GLOVES(("gauntlets of power", "riding gloves"),
		0, 1,  7, 1, 30, 50,  9, 1, 0, IRON, CLR_BROWN),
...
/* boots */
//define BOOTS(names,kn,mgc,prob,delay,wt,cost,ac,dr,can,metal,c) \
	ARMOR(names,kn,mgc,0,prob,delay,wt,cost,ac,dr,can,ARM_BOOTS,metal,c)
BOOTS(("low boots", "walking shoes"),
		0, 0,  25, 2, 10,  8, 10, 1, 0, LEATHER, HI_LEATHER),
BOOTS(("shoes", "hard shoes"),
		0, 0,   7, 2, 50, 16,  9, 1, 0, IRON, HI_METAL, O_MATSPEC(IDED)),
BOOTS(("armored boots", "boots"),
		0, 0,   0, 1, 75, 16,  8, 4, 1, IRON, HI_METAL, O_MATSPEC(IDED|UNIDED)),
...
BOOTS(("stilettos", "high-heeled shoes"), /*Needs encyc entry*//*Needs tile*/
		0, 0,   0, 1, 10, 60, 10, 0, 0, METAL, HI_METAL),
...
BOOTS(("speed boots", "combat boots"),
		0, 1,  12, 2, 20, 50,  9, 0, 0, LEATHER, HI_LEATHER, O_POWER(FAST)),
BOOTS(("water walking boots", "jungle boots"),
		0, 1,  12, 2, 20, 50, 10, 1, 0, LEATHER, HI_LEATHER, O_POWER(WWALKING)),
BOOTS(("jumping boots", "hiking boots"),
		0, 1,  12, 2, 20, 50,  9, 1, 0, LEATHER, HI_LEATHER, O_POWER(JUMPING)),
BOOTS(("elven boots", "mud boots"),
		0, 1,  12, 2, 10,  8,  9, 0, 0, WOOD, CLR_BROWN, O_POWER(STEALTH)),
BOOTS(("kicking boots", "buckled boots"),
		0, 1,  12, 2, 15,  8,  9, 2, 0, IRON, CLR_BROWN),
...
TOOL(("anvil"),        1,  MZ_MEDIUM, 0, 0, 0,  0, 500, 50, IRON, HI_METAL),
#endif
...
/* potions ... */
#define POTION(names,mgc,prob,cost,color,...) OBJECT( \
		DEF_BLINDNAME(names, "potion"), BITS(0,1,0,0,mgc,0,0,0,MZ_SMALL,0,0,0,0,P_NONE,GLASS,0), {0}, \
...
POTION(("confusion", "orange"),           1, 33, 100, CLR_ORANGE),
POTION(("blindness", "yellow"),           1, 33, 150, CLR_YELLOW),
POTION(("paralysis", "emerald"),          1, 32, 300, CLR_BRIGHT_GREEN),
POTION(("speed", "dark green"),           1, 40, 200, CLR_GREEN),
POTION(("levitation", "cyan"),            1, 40, 200, CLR_CYAN),
POTION(("hallucination", "sky blue"),     1, 40, 100, CLR_CYAN),
POTION(("invisibility", "brilliant blue"),1, 40, 150, CLR_BRIGHT_BLUE),
POTION(("see invisible", "magenta"),      1, 40,  50, CLR_MAGENTA),
...
POTION(("gain energy", "cloudy"),         1, 40, 150, CLR_WHITE),
POTION(("sleeping",  "effervescent"),     1, 40, 100, CLR_GRAY),
...
POTION(("acid", "white"),                 0, 32, 250, CLR_WHITE),
